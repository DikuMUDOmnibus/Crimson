...
struct shop_data{
  ...
  char stocked; /* initially FALSE - dont want to repetitively stock a shopkeeper */
};
...
/* Crimson addition: if the shopkeeper dont have an infinite item, give it to 'em */
void shop_stock(struct char_data *keeper, int shop_nr)
{
  int counter;
  struct obj_data *tmp_obj;
  
  if(shop_index[shop_nr].stocked) return;
  shop_index[shop_nr].stocked = TRUE; /* will soon be stocked */
  
  for(counter=0;counter<MAX_PROD;counter++)
    if (!(tmp_obj = get_obj_in_list_num(shop_index[shop_nr].producing[counter],keeper->carrying))) {
      if(shop_index[shop_nr].producing[counter] >= 0)
        if((tmp_obj = read_object(shop_index[shop_nr].producing[counter], REAL)))
          obj_to_char(tmp_obj, keeper);
    }
}
		  ...
		  struct char_data *keeper, int shop_nr)
{
  ...
  if(GET_GOLD(ch) < (int) (temp1->obj_flags.cost*
			   shop_index[shop_nr].profit_buy) && GET_LEVEL(ch)<IMO_LEV2)
    {
      sprintf(buf,
  ...
  ansi_act("$n buys $p.", FALSE, ch, temp1, 0, TO_ROOM,CLR_ACTION);
		   ...
		   struct char_data *keeper,int shop_nr)
{
  ...
  if (IS_SET(temp1->obj_flags.extra_flags, ITEM_NODROP))
    {
      ansi(CLR_ERROR, ch);
  ...
  ansi_act("$n sells $p.", FALSE, ch, temp1, 0, TO_ROOM,CLR_ACTION);
...
int shop_keeper(struct char_data *ch, int cmd, char *arg)
{
  ...
  if (cmd >= 56 && cmd <= 59 
      && IS_GOOD(keeper) 
      && !IS_NPC(ch) && IS_SET(ch->specials.act, PLR_PKILLER) ){
            ansi_act("$N tells you, 'I dont deal with MURDERERS!!'", FALSE, ch, 0, keeper, TO_CHAR,CLR_COMM);
		return(TRUE);
	      }
  ...
  if ((cmd==25) || (cmd==70) || (cmd==159) || (cmd==157))   
    /* Kill or Hit or Kick or Bash */
	{
	  one_argument(arg, argm);
		...
		ansi_act("$N tells you, 'No magic here - kid!'", FALSE, ch, 0, keeper, TO_CHAR,CLR_COMM);
		return TRUE;
	      }
