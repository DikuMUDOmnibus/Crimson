...
extern char *str_alloc(char *source);
extern char *str_free(char *source);
extern int str_cmp(char *arg1, char *arg2);
extern int strn_cmp(char *arg1, char *arg2, int n);
extern void log(char *str);
extern void sprinttype(int type, const char *names[], char *result);
extern void sprintbit(long vektor, const char *names[], char *result);
...
#define IS_DARK(room) (!world[room].light &&                                                         (IS_SET(world[room].room_flags, DARK) ||                                       ((weather_info.sunlight == SUN_DARK) &&                                        (!IS_SET(world[room].room_flags, INDOORS)) &&                                  (world[room].sector_type > SECT_CITY))))
...
#define CAN_SEE(sub, obj) \
         ( (GET_LEVEL((sub))>=42) || \
           ( (!IS_AFFECTED((obj),AFF_INVISIBLE) || IS_AFFECTED((sub),AFF_DETECT_INVISIBLE)) && \
             (!IS_AFFECTED((obj),AFF_WIZINVIS)) && \
             (!IS_AFFECTED((sub),AFF_BLIND)) && \
             (IS_AFFECTED((sub), AFF_DARKSIGHT) || !IS_DARK(sub->in_room)) \
           ) \
         )
...
#define CAN_CARRY_W(ch) (str_app[MAXV(0, STRENGTH_APPLY_INDEX(ch) - \
			                MAXV(0, age(ch).year /10) - \
			                MAXV(0, (16 - age(ch).year)) \
				)].carry_w \
			)
